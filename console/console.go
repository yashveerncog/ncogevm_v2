...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"nec": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) Welcome() {
	message := "Welcome to the Forest JavaScript console!\n\n"

	// Print some generic Geth metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + nec.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + nec.blockNumber + " (" + new Date(1000 * nec.getBlock(nec.blockNumber).timestamp) + ")\n";
		try {
